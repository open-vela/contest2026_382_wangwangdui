...
- 目标板资源受限时的页面复杂度控制，例如减少高频重绘、降低应用列表动画负担、保留可读 UI 和稳定路由优先级。
...
部分能力依赖目标硬件和 Vela native feature 支持。硬件或系统 feature 不可用时，应用会使用降级逻辑或演示数据。健康、运动、趋势等页面当前以演示数据为主，重点验证页面结构、路由、状态管理和低资源设备上的可用交互。

## 真机能力限制与降级

当前目标硬件和系统镜像并不保证提供所有 watch native feature。部分手表类能力在模拟器或完整系统中可以由软件模拟，但在当前开发板上受限于板载传感器、外设连接和 native feature 暴露范围，需要采用演示数据或保守降级。已知验证情况如下：

- 亮度页：UI 和设置项可用；若系统未提供 `system.brightness` 或底层背光控制未接入，滑动条只保存状态，不保证改变真实屏幕亮度。
- 振动页：若硬件或系统未提供 `system.vibrator`，页面保留交互和提示，不触发真实马达。
- 运动 / 心率 / 趋势：当前开发板未完整提供心率、运动健康等手表传感器链路；若无健康传感器或 `service.health`，使用模拟数据或降级数据展示。
- 加速度 / 动作诊断：若缺少 `system.sensor` 或对应 IMU 通道，页面展示兼容状态，不强依赖真实加速度数据。
...
模拟器和 PC 端预览环境通常具有更高的 CPU、内存、文件系统和图形处理能力，页面切换、动画和列表滚动会比开发板更顺滑。当前目标板在运行 Quick App、LVGL framebuffer、触摸输入和应用 JS 逻辑时共享有限资源，因此真实交互性能更接近低功耗穿戴设备的边界条件。

本交付优先保证：

- 真机可启动和主要页面可达。
- 表盘、应用列表、设置、健康、运动等核心路径稳定。
- 硬件能力缺失时不阻塞页面路由。
- 数据不可用时使用可识别的演示数据或默认值。
- 在低资源条件下减少不必要的动态效果和高频刷新。

因此，若评审时看到部分页面在真机上的流畅度低于模拟器，应优先结合目标板资源、Quick App 运行时和 native feature 支持范围进行判断；源码和 RPK 已保留对应的降级逻辑，便于在更完整的手表硬件或系统 feature 集上继续接入真实传感器与设备能力。
