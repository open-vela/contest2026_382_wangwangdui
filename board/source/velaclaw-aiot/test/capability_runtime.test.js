...
assert.ok(healthChannel.includes("getNativeFeature('@service.health')"), 'health raw API must be safely loaded in capability runtime')
...
assert.ok(motion.includes("getNativeFeature('@system.sensor')"), 'motion gateway must own accelerometer API')
assert.ok(location.includes("getNativeFeature('@system.geolocation')"), 'location gateway must own geolocation API')
assert.ok(device.includes("from '@system.device'"), 'device gateway must own device API')
assert.ok(battery.includes("getNativeFeature('@system.battery')"), 'battery gateway must own battery API')
assert.ok(displayPower.includes("getNativeFeature('@system.brightness')"), 'display gateway must own brightness API')
assert.ok(vibration.includes("getNativeFeature('@system.vibrator')"), 'vibration gateway must own vibrator API')
assert.ok(!storage.includes('@system.storage'), 'storage gateway must avoid the broken device DB on this target')
assert.ok(storage.includes('memoryCache') && storage.includes('memoryOnly'), 'storage gateway must keep a memory fallback')
assert.ok(systemEvent.includes("getNativeFeature('@system.event')"), 'event gateway must own system event API')
assert.ok(interconnect.includes("getNativeFeature('@system.interconnect')"), 'interconnect gateway must own interconnect API')
