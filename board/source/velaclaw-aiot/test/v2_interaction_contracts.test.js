...
Object.keys(manifest.router.pages || {}).forEach(route => {
  ...
  if (ownsScroll) {
    ...
    if (ownsSwipe) {
      assert.ok(source.includes("event.direction === 'right'") && source.includes('navigation.back()'), relative + ' scroll/list swipe may only provide right-swipe back navigation')
    }
  }
})
...
assert.strictEqual(count(clock, '@swipe='), 0, 'Clock must not combine native swipe with raw touch gestures')
assert.strictEqual(count(clock, '@longpress='), 0, 'Clock long-press must be cancelled by raw touch movement')
...
assert.ok(clock.includes('this.longPressTimer = setTimeout') && clock.includes('this.clearLongPressTimer()'), 'Clock long-press must be owned by the raw touch sequence')
assert.ok(clock.includes('Math.max(Math.abs(dx), Math.abs(dy)) > 10'), 'Clock movement must cancel pending face-selector long-press')
assert.ok(clock.includes('consumeGestureEvent(event)'), 'Clock gestures must consume handled touch sequences')
...
assert.ok(launcher.includes("event.direction === 'left') this.controller.next()"), 'Launcher left-swipe must always advance through circular pages')
assert.ok(launcher.includes("event.direction === 'right') this.controller.previous()"), 'Launcher right-swipe must always wrap to the previous circular page')
assert.ok(!launcher.includes('this.hasPrevious) this.controller.previous()') && !launcher.includes("item.id === 'faces') navigation.replace(route)"), 'Launcher paging and face selector routing must not fall back through Clock')
