...
function api() {
  if (!sensor) sensor = getNativeFeature('@system.sensor')
  return sensor
}
...
function stopNative() {
  ...
  try {
    var native = api()
    if (native && native.unsubscribeAccelerometer) native.unsubscribeAccelerometer()
  } catch (error) {}
  active = false
  activeInterval = ''
}
...
function startNative(interval) {
  ...
  try {
    var native = api()
    if (!native || !native.subscribeAccelerometer) {
      emitError('unavailable')
      return false
    }
    active = true
    activeInterval = interval || 'normal'
    native.subscribeAccelerometer({
...
export default {
  ...
  isAvailable: function () { var native = api(); return !!(native && native.subscribeAccelerometer) }
}
