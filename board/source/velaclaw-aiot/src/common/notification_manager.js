...
var notificationManager = {
  ...
  setupEventListener() {
    ...
    var self = this
    try {
      eventListener = function (res) {
        self.handleConsoleEvent(res)
      }
      if (systemEvent.subscribe(NOTIFICATION_EVENT_NAME, eventListener)) {
  ...
  teardownEventListener() {
    ...
    try {
      if (eventListener) systemEvent.unsubscribe(NOTIFICATION_EVENT_NAME, eventListener)
    } catch (e) {
  ...
  setupInterconnect() {
    ...
    var self = this
    try {
      if (!interconnect.isAvailable()) return
      interconnectListener = function (msg) {
        self.handleInterconnectMessage(msg)
      }
      interconnect.subscribe(interconnectListener)
  ...
  destroy() {
    ...
    try {
      if (interconnectReady && interconnectListener) interconnect.unsubscribe(interconnectListener)
    } catch (e) {}
    interconnectReady = false
    interconnectListener = null
    changeCallback = null
  }
}
