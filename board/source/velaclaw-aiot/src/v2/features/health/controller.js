...
export function createHealthController(onChange) {
  ...
  function available(data, prefix) {
    ...
    if (data[prefix + 'Live'] && data[prefix + 'Source'] === 'live') return true
    return data[prefix] !== undefined && data[prefix] !== null && data[prefix + 'Source'] === 'fallback'
  }

  function updateWindow(values, changed, value, allowed) {
    var number = Number(value)
    if ((!changed && values.length) || !allowed || !isFinite(number)) return values
    return healthMetrics.pushWindow(values, Math.round(number), 10)
  }

  function seedCurrent(data) {
    if (!data) return
    if (!heartValues.length && available(data, 'heartRate') && Number(data.heartRate) > 0) heartValues = [Math.round(Number(data.heartRate))]
    if (!spo2Values.length && available(data, 'spo2') && Number(data.spo2) > 0) spo2Values = [Math.round(Number(data.spo2))]
    if (!stressValues.length && available(data, 'stress') && isFinite(Number(data.stress)) && Number(data.stress) >= 0) stressValues = [Math.round(Number(data.stress))]
  }
