...
var ICON_BASE = 34
var ICON_GROW = 16
var ICON_RENDER_SIZE = 48
var ICON_RENDER_RADIUS = Math.round(ICON_RENDER_SIZE / 2)
...
function scaleTransform(size) {
  var scale = Math.round((Number(size) || ICON_RENDER_SIZE) / ICON_RENDER_SIZE * 100) / 100
...
function buildSlots(apps) {
  ...
  for (var index = 0; index < source.length; index++) {
    ...
    slots.push({
      ...
      accent: app.accent,
      glyph: app.glyph,
      glyphSize: app.glyphSize,
      glyphColor: app.glyphColor,
      liteIcon: app.liteIcon || app.icon,
      normalIcon: app.liteIcon || app.normalIcon || app.icon,
      softIcon: app.liteIcon || app.softIcon || app.normalIcon || app.icon,
      sourceIndex: index,
      q: coordinate.q,
      r: coordinate.r,
      gridX: coordinate.x,
...
function layoutSlots(slots, panX, panY, offsetX, offsetY, activeIcons) {
  var source = slots || []
  var nextSlots = []
  var nearestIndex = -1
  var nearestDistance = Infinity
  var showActiveIcon = activeIcons !== false
  for (var index = 0; index < source.length; index++) {
    ...
    nextSlots.push({
      ...
      accent: slot.accent,
      glyph: slot.glyph,
      glyphSize: slot.glyphSize,
      glyphColor: slot.glyphColor,
      liteIcon: slot.liteIcon,
      ...
      size: point.size,
      radius: ICON_RENDER_RADIUS,
      renderSize: ICON_RENDER_SIZE,
      transform: scaleTransform(point.size),
      opacity: opacity,
      icon: slot.liteIcon || slot.normalIcon,
      left: Math.round(point.centerX - ICON_RENDER_SIZE / 2),
      top: Math.round(point.centerY - ICON_RENDER_SIZE / 2),
...
function visibleSlots(slots, limit) {
  var source = slots || []
  var maxCount = Math.max(0, Math.floor(Number(limit) || 0))
  if (!maxCount && source.length <= DEFAULT_COORD_COUNT && source[0] && source[0].slotKey !== undefined) return source.slice()
  var result = []
  ...
  if (!maxCount || result.length <= maxCount) return result
  result.sort(function (a, b) { return a.distance - b.distance })
  result = result.slice(0, maxCount)
  result.sort(function (a, b) { return a.sourceIndex - b.sourceIndex })
  return result
}
...
function clampPan(slots, panX, panY, overscroll, cachedBounds) {
  var bounds = cachedBounds || panBounds(slots)
...
function nextPan(slots, panX, panY, deltaX, deltaY, cachedBounds) {
  var bounds = cachedBounds || panBounds(slots)
  ...
  return clampPan(slots, nextX, nextY, OVERSCROLL_LIMIT, bounds)
}
...
module.exports = {
  ...
  DRAG_CAPTURE_DISTANCE: DRAG_CAPTURE_DISTANCE,
  MAX_FRAME_DELTA: MAX_FRAME_DELTA,
  FRAME_MS: FRAME_MS,
  DRAG_FRAME_MS: DRAG_FRAME_MS,
